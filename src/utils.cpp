...
/*namespace vili
{
    namespace Functions
    {
        ...
        void String::replaceStringInPlace(
            std::string& subject, const std::string& search, const std::string& replace)
        {
            size_t pos = 0;
            while ((pos = subject.find(search, pos)) != std::string::npos)
        ...
        std::string String::replaceString(
            std::string subject, const std::string& search, const std::string& replace)
        {
            size_t pos = 0;
            while ((pos = subject.find(search, pos)) != std::string::npos)
        ...
        std::vector<std::string> String::extractBetween(
            std::string& str, char delimiter1, char delimiter2)
        {
            ...
            for (unsigned int i = 0; i < string.size(); i++)
            {
                ...
                if (currentChar == "\"")
                {
                    if (readingString)
                    {
                        ...
                        indexes.push_back(
                            std::pair<int, int>(0, extractedStrings.size() - 1));
                    }
                    else if (currentStack.size() > 0)
                    {
                        otherComponents.push_back(currentStack);
                        indexes.push_back(
                            std::pair<int, int>(1, otherComponents.size() - 1));
                    }
        ...
        //Functions::Vector
        std::string Vector::join(const std::vector<std::string>& vector,
            const std::string& sep, int start, int end)
        {
            std::string result = "";
            if (end >= vector.size())
                end = vector.size();
        ...
        void Vector::joinBetween(std::vector<std::string>& vector,
            const std::string& joinValue, const std::string& sep)
        {
            std::string stack = "";
            bool stacking = false;
        ...
        void Vector::mergeNeighboors(std::vector<std::string>& vector,
            const std::string& n1, const std::string& n2, const std::string& sep,
            bool strict)
        {
            if (vector.size() > 0)
            {
                ...
                for (unsigned int i = 0; i < vector.size() - 1; i++)
                {
                    ...
                    {
                        if ((vector.at(i) == n1 && vector.at(i + 1) == n2)
                            || (vector.at(i) == n2 && vector.at(i + 1) == n1))
                        {
                            vector.at(i) = vector.at(i) + sep + vector.at(i + 1);
                            toErase.push_back(i + 1);
                        }
